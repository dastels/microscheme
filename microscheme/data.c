...
#ifdef DEBUG_TRACE
    char * str = to_string(d);
    printf("Retaining a %s: %s. Reference count now %d\n", type_name(type_of(d)), str, d->meta.refs);
    free(str);
#endif
  }
  return d;
...
#ifdef DEBUG_TRACE
    char * str = to_string(d);
    printf("Reference count = 0. Freeing a %s: %s\n", type_name(type_of(d)), str);
    free(str);
#endif
    switch (type_of(d)) {
    case STRING_TYPE:
...
#ifdef DEBUG_TRACE
    printf("Decremented ref count of a %s: %s Now %d.\n", type_name(type_of(d)), to_string(d), d->meta.refs);
#endif
  }
}
...
#ifdef DEBUG_TRACE
  printf("Allocating a %s.\n", type_name(the_type));
#endif

  if (free_list == NULL) {
#ifdef DEBUG_TRACE
    printf("Could not allocate data object\n");
#endif
    exit(-1);
  }
  data_t *d = free_list;
...
#ifdef DEBUG_TRACE
  printf("Pre allocating %d small integers.\n", SMALL_INTEGER_CACHE_SIZE);
#endif
...
char *to_string(data_t *d)
{
  ...
  switch (type_of(d)) {
  case FREE_TYPE:
    return strdup("Free-object");
  case CONS_CELL_TYPE:
    return list_to_string(d);
  case INTEGER_TYPE:
    return int_to_string(d);
  case UNSIGNED_INTEGER_TYPE:
    return uint_to_string(d);
  case BOOLEAN_TYPE:
    return bool_to_string(d);
  case STRING_TYPE:
    return string_to_string(d);
  case SYMBOL_TYPE:
    return strdup(string_value(d));
  case FUNCTION_TYPE:
    return func_to_string(d);
  case MACRO_TYPE:
    return macro_to_string(d);
  case PRIMITIVE_TYPE:
    return prim_to_string(d);
  default:
    return strdup("unknown data type");
  }
}
